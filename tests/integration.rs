...
async fn creates_and_updates_metadata_schema() {
    ...
    client
        .datasets()
        .create(CreateDatasetRequest::builder("products").metadata_schema(schema.clone()))
        .await
        .expect("created");
    client
        .datasets()
        .patch_metadata_schema("ds_1", schema.clone())
        .await
        .expect("merged");
    client
        .datasets()
        .replace_metadata_schema("ds_1", vec![])
        .await
        .expect("replaced");
    ...
    assert_eq!(
        merge,
        json!({"schema":[{"name":"price", "type":"f32"}], "mode":"merge"})
    );
